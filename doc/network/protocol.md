...
- [网络：协议](#网络协议)
  ...
  - [传输层](#传输层)
    - [TCP 消息头](#tcp-消息头)
    - [TCP 的建立](#tcp-的建立)
      - [TCP 第三次握手失败会怎么办？](#tcp-第三次握手失败会怎么办)
    - [TCP 连接断开](#tcp-连接断开)
    - [TCP 客户端主动断开链接时，客户端为什么还要等待 2MSL？](#tcp-客户端主动断开链接时客户端为什么还要等待-2msl)
    - [TCP 的重传](#tcp-的重传)
    - [TCP 粘包、拆包及解决办法](#tcp-粘包-拆包及解决办法)
    - [TCP 拥塞控制的 4 中算法：慢启动，拥塞避免，快重传，快恢复](#tcp-拥塞控制的-4-中算法慢启动拥塞避免快重传快恢复)
      - [慢启动](#慢启动)
      - [拥塞避免算法](#拥塞避免算法)
      - [快重传](#快重传)
      - [快恢复](#快恢复)
...
### TCP 客户端主动断开链接时，客户端为什么还要等待 2MSL？

1. 第四次 ACK 可能会发送失败，如果服务端没有收到 ACK,会重传第三次 FIN，如果客户端已经关闭了，就会导致 RST。所以要等待。
...
### TCP 拥塞控制的 4 中算法：慢启动，拥塞避免，快重传，快恢复
