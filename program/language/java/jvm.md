...
- [1. 基础模块](#1-基础模块)
  - [1.1. 创建一个空 Object 占用多少内存](#11-创建一个空-object-占用多少内存)
- [2. 类加载器模块](#2-类加载器模块)
  - [2.1. 类加载器分类](#21-类加载器分类)
  - [2.2. 类加载机制一:全盘委托](#22-类加载机制一全盘委托)
  - [2.3. 类加载机制二：双亲委派](#23-类加载机制二双亲委派)
  - [2.4. 类加载流程](#24-类加载流程)
  - [2.5. ClassNotFoundException 和 NoClassDefFoundException 的区别？](#25-classnotfoundexception-和-noclassdeffoundexception-的区别)
- [3. jvm 内存结构](#3-jvm-内存结构)
  - [3.1. jvm 内存划分](#31-jvm-内存划分)
  - [3.2. jvm 内存为什么要分代？](#32-jvm-内存为什么要分代)
    - [3.2.1. 新生代（Young）](#321-新生代young)
    - [3.2.2. 老年代（Old）](#322-老年代old)
    - [3.2.3. 永久代（Permanent）](#323-永久代permanent)
    - [3.2.4. Minor GC 和 Full GC 的区别](#324-minor-gc-和-full-gc-的区别)
- [4. GC](#4-gc)
  - [4.1. 判断对象是否存活的方法](#41-判断对象是否存活的方法)
  - [4.2. jvm 垃圾回收算法](#42-jvm-垃圾回收算法)
  - [4.3. Minor GC、Major GC、Full GC、mixed gc 是什么？](#43-minor-gc-major-gc-full-gc-mixed-gc-是什么)
  - [4.4. jvm 中的垃圾回收器](#44-jvm-中的垃圾回收器)
    - [4.4.1. Serial 收集器（新生代，复制算法）](#441-serial-收集器新生代复制算法)
    - [4.4.2. ParNew 收集器（新生代，复制算法）](#442-parnew-收集器新生代复制算法)
    - [4.4.3. Parallel Scavenge 收集器（新生代，复制算法）](#443-parallel-scavenge-收集器新生代复制算法)
    - [4.4.4. Serial Old 收集器（老年代，标记-整理算法）](#444-serial-old-收集器老年代标记-整理算法)
    - [4.4.5. Parallel Old 收集器（老年代，标记-整理算法）](#445-parallel-old-收集器老年代标记-整理算法)
    - [4.4.6. CMS 收集器（老年代，标记-清除算法）](#446-cms-收集器老年代标记-清除算法)
    - [4.4.7. CMS 收集器的缺点：](#447-cms-收集器的缺点)
    - [4.4.8. 为什么 CMS 两次标记时要 stop the world](#448-为什么-cms-两次标记时要-stop-the-world)
    - [4.4.9. G1 收集器（标记-整理）：优先处理那些垃圾多的内存块](#449-g1-收集器标记-整理优先处理那些垃圾多的内存块)
    - [4.4.10. G1 收集器是如何做到可预测的停顿的？](#4410-g1-收集器是如何做到可预测的停顿的)
    - [4.4.11. Parallel scavenge 收集器与 ParNew 收集器的区别？](#4411-parallel-scavenge-收集器与-parnew-收集器的区别)
    - [4.4.12. CMS 和 G1 收集器的区别？](#4412-cms-和-g1-收集器的区别)
    - [4.4.13. 什么是 GC ROOT，GC 如何找到死亡的对象？](#4413-什么是-gc-rootgc-如何找到死亡的对象)
    - [4.4.14. 触发 FullGC 的场景](#4414-触发-fullgc-的场景)
  - [4.5. 触发内存泄漏的场景](#45-触发内存泄漏的场景)
  - [4.6. 触发栈内存溢出的场景](#46-触发栈内存溢出的场景)
- [5. JVM 调优](#5-jvm-调优)
  - [5.1. JVM 相关工具](#51-jvm-相关工具)
  - [5.2. java 命令常用参数](#52-java-命令常用参数)
- [6. 线上常见问题定位](#6-线上常见问题定位)
  - [6.1. 如何定位 cpu 100%问题？](#61-如何定位-cpu-100问题)
  - [6.2. 如何定位内存 100%问题？](#62-如何定位内存-100问题)

<!-- /code_chunk_output -->

## 1. 基础模块

### 1.1. 创建一个空 Object 占用多少内存
...
### 2.5. ClassNotFoundException 和 NoClassDefFoundException 的区别？
...
### 4.3. Minor GC、Major GC、Full GC、mixed gc 是什么？
...
#### 4.4.1. Serial 收集器（新生代，复制算法）

单线程收集器，收集垃圾时会 stop the world

#### 4.4.2. ParNew 收集器（新生代，复制算法）
...
#### 4.4.3. Parallel Scavenge 收集器（新生代，复制算法）

并发的多线程收集器，追求高吞吐量，高效利用 CPU，吞吐量一般为 99%（100 分钟一分钟在回收垃圾）

#### 4.4.4. Serial Old 收集器（老年代，标记-整理算法）

Serial 收集器的老年代版，单线程收集

#### 4.4.5. Parallel Old 收集器（老年代，标记-整理算法）

Paralle Scavenge 的老年代版本

#### 4.4.6. CMS 收集器（老年代，标记-清除算法）
...
#### 4.4.8. 为什么 CMS 两次标记时要 stop the world

- CMS 并非没有暂停，而是用两次短暂停来替代串行标记整理算法的长暂停。

#### 4.4.9. G1 收集器（标记-整理）：优先处理那些垃圾多的内存块
...
#### 4.4.11. Parallel scavenge 收集器与 ParNew 收集器的区别？

parallel scavenge 有垃圾自适应调节策略。

#### 4.4.12. CMS 和 G1 收集器的区别？
...
#### 4.4.13. 什么是 GC ROOT，GC 如何找到死亡的对象？
