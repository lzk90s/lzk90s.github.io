...
- [2. 索引模块](#2-索引模块)
  ...
  - [2.7. 索引分类](#27-索引分类)
    ...
    - [2.7.2. 非主键索引的 B+树叶子节点存储的什么？](#272-非主键索引的-b树叶子节点存储的什么)
...
- [4. 锁模块](#4-锁模块)
  - [4.1. 表锁](#41-表锁)
  - [4.2. 行锁（锁存在的记录）](#42-行锁锁存在的记录)
  - [4.3. 间隙锁 GapLock（锁不存在的记录，阻塞 insert 操作）](#43-间隙锁-gaplock锁不存在的记录阻塞-insert-操作)
  - [4.4. nex-key lock（间隙锁和行锁的合集称）](#44-nex-key-lock间隙锁和行锁的合集称)
    - [4.4.1. next -key lock 影响并发怎么办？](#441-next-key-lock-影响并发怎么办)
  - [4.5. update、delete、select for update 什么条件加间隙锁？](#45-update-delete-select-for-update-什么条件加间隙锁)
...
- [6. 性能优化](#6-性能优化)
  ...
  - [6.3. mysql 读写分离](#63-mysql-读写分离)
  - [6.4. mysql 分库分表](#64-mysql-分库分表)
  - [6.5. mysql JOIN 的实现（Nested-Loop Join（嵌套循环连接））](#65-mysql-join-的实现nested-loop-join嵌套循环连接)
  - [6.6. 分区&分库&分表](#66-分区分库分表)
- [7. 参考](#7-参考)
...
### 4.3. 间隙锁 GapLock（锁不存在的记录，阻塞 insert 操作）

- 间隙锁锁住的是行之间的空隙

### 4.4. nex-key lock（间隙锁和行锁的合集称）

#### 4.4.1. next -key lock 影响并发怎么办？

间隙锁的引入，虽然解决了幻读的问题，但同时也降低了并发度，而且可能会导致死锁。
假如业务需求并不需要间隙锁怎么办，这时可以将隔离级别 RC，在此级别下就不存在间隙锁了。

### 4.5. update、delete、select for update 什么条件加间隙锁？
...
### 6.5. mysql JOIN 的实现（Nested-Loop Join（嵌套循环连接））
