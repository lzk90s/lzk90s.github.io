...
- [1. redis 基础](#1-redis-基础)
  - [1.1. redis 和 memcached 的区别？](#11-redis-和-memcached-的区别)
  - [1.2. redis 高性能的原因](#12-redis-高性能的原因)
  - [1.3. redis lua](#13-redis-lua)
  - [1.4. redis 客户端 jedis 和 redisson 有哪些区别？](#14-redis-客户端-jedis-和-redisson-有哪些区别)
- [2. redis 数据类型](#2-redis-数据类型)
  - [2.1. redis 的数据类型](#21-redis-的数据类型)
  - [2.2. redis 数据类型底层存储实现](#22-redis-数据类型底层存储实现)
  - [2.3. redis 存储结构 ziplist](#23-redis-存储结构-ziplist)
  - [2.4. redis 存储结构 skiplist](#24-redis-存储结构-skiplist)
  - [2.5. redis 选择 hash 还是 string 存储数据？](#25-redis-选择-hash-还是-string-存储数据)
- [3. 持久化](#3-持久化)
  - [3.1. RDB](#31-rdb)
  - [3.2. AOF](#32-aof)
- [4. 内存淘汰](#4-内存淘汰)
  - [4.1. redis 内存淘汰机制](#41-redis-内存淘汰机制)
  - [4.2. LRU（Least Recently Used：最近最少使用）是什么](#42-lruleast-recently-used最近最少使用是什么)
  - [4.3. LRU 的实现方法](#43-lru-的实现方法)
- [5. 典型使用场景](#5-典型使用场景)
  - [5.1. redis 做缓存](#51-redis-做缓存)
    - [5.1.1. 如何保证缓存和数据库的双写一致性？](#511-如何保证缓存和数据库的双写一致性)
  - [5.2. Redis 如何做异步队列？](#52-redis-如何做异步队列)
  - [5.3. redis 如何实现延时队列?](#53-redis-如何实现延时队列)
  - [5.4. redis 如何实现分布式锁？](#54-redis-如何实现分布式锁)
  - [5.5. redis 存储 1 亿的用户状态？](#55-redis-存储-1-亿的用户状态)
- [6. redis 使用常见问题](#6-redis-使用常见问题)
  - [6.1. 缓存穿透是什么?](#61-缓存穿透是什么)
    - [6.1.1. 布隆过滤器是什么？](#611-布隆过滤器是什么)
  - [6.2. 缓存击穿是什么？](#62-缓存击穿是什么)
  - [6.3. 缓存雪崩是什么？](#63-缓存雪崩是什么)
- [7. 集群模式](#7-集群模式)
  - [7.1. redis 集群模式的 一致性 hash 算法](#71-redis-集群模式的-一致性-hash-算法)

<!-- /code_chunk_output -->

## 1. redis 基础

### 1.1. redis 和 memcached 的区别？
...
### 1.4. redis 客户端 jedis 和 redisson 有哪些区别？
...
### 2.5. redis 选择 hash 还是 string 存储数据？
...
RDB（Redis database 内存快照）持久化：保存某个时间点的全量数据快照，分为手动触发和自动触发。
优点：全量数据快照，文件小，恢复快，
缺点：最近一次数据可能会丢失，数据量大有瓶颈。

### 3.2. AOF

AOF（Append Only File 日志文件）持久化：记录每次对服务器写的操作，当服务器重启的时候会重新执行。
...
### 4.2. LRU（Least Recently Used：最近最少使用）是什么
